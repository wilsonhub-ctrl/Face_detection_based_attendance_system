...
def TrackImages():
    ...
    if exists1:
        ...
        cv2.destroyAllWindows()
        window.destroy()
    marked_ids = set()  # To track IDs already marked in this session
    attendance_records = []  # List to collect attendance records
    recognized = False  # Flag to auto-close camera after recognition
    ...
    while time.time() - start_time < 5:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                if serial not in marked_ids:
                    ...
                    recognized = True  # Set flag to close camera
                    time.sleep(2)  # Show the recognized name for 2 seconds
                else:
        ...
        window.update()
        if (cv2.waitKey(1) == ord('q')) or recognized:
            break
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile("Attendance/attendence.csv")
    with open("Attendance/attendence.csv", 'a+') as csvFile1:
        writer = csv.writer(csvFile1)
        if not exists:
